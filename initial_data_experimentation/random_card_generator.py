...
AVG_TRADES = np.random.uniform(1, 10)
...
def generate_mock_card_data(n_cards=SAMPLE_SIZE, n_days=TIMEFRAME, start_date=START_DATE, seed=SEED):
    ...
    def generate_trade_history(price_series, trade_rate, base_date):
        sales = []
        for day_index, price in enumerate(price_series):
            ...
            for _ in range(n_trades):
                ...
                sales.append({"date": trade_date, "price": noisy_price})
        return sales
    ...
    for i in range(n_cards):
        ...
        sales = generate_trade_history(price_series, avg_trades, base_date)
